...
const Cart: React.FC = () => {
  ...
  // ✨ De tu versión local: Blindaje contra errores de stock
  const carritoTieneErrores = cart.some(item => item.cantidad > (item.stock_local || 0));
  
  // 💸 De la rama de Rogelio: Navegación de Checkout e Impuestos
  const [mostrarPago, setMostrarPago] = useState(false);
  const subtotalFiscal = totalPrice / 1.16;
  const impuestos = totalPrice - subtotalFiscal;
      ...
      <main className="grow max-w-1440px mx-auto w-full px-4 sm:px-6 md:px-16 py-8 md:py-12">
        ...
        {cart.length === 0 ? (
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            {/* COLUMNA IZQUIERDA: LISTA O CHECKOUT */}
            <div className="w-full lg:w-2/3">
              {!mostrarPago ? (
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 animate-fade-in">
                  ...
                  <div className="divide-y divide-gray-100">
                    {cart.map((item, index) => {
                      ...
                      // Combinamos los fallbacks de imagen de ambas ramas
                      ...
                      const categoriaSegura = prod.nombre_categoria || prod.id_categoria || 'General';
                      
                      // Mantenemos tu lógica robusta de precios, descuentos y stock local
                      ...
                      return (
                        <div key={productId} className={`py-6 grid grid-cols-1 md:grid-cols-12 gap-4 items-center relative transition-opacity ${stockDisponible === 0 ? 'opacity-80' : ''}`}>
                          ...
                          {/* ✨ ALERTA DE STOCK MEJORADA (Mantiene tu validación visual) */}
                          {hayConflictoStock && (
                          ...
                          {/* COLUMNA 1: Imagen, Nombre y Precio Unitario */}
                ...
                <div className="animate-fade-in">
                  <h2 className="text-2xl font-bold text-slate-900 mb-4">Completar Pago</h2>
                  {/* Flujo de pago importado de Rogelio */}
                  <Checkout />
                </div>
              )}
            </div>
            ...
            {/* ========================================================= */}
            {/* TARJETA DE RESUMEN CON DESGLOSE DE IMPUESTOS Y BOTONES UNIFICADOS */}
            {/* ========================================================= */}
            <div className="w-full lg:w-1/3 bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 sticky top-24">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between text-gray-600">
                  ...
                  <span className="font-medium">${subtotalFiscal.toFixed(2)}</span>
                </div>
                
                {/* 👇 Desglose del IVA (De Rogelio) 👇 */}
                <div className="flex justify-between text-gray-600">
                  <div className="flex items-center gap-1">
                    <span>IVA</span>
                    <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-400 font-bold">16%</span>
                  </div>
              ...
              <div className="border-t border-gray-100 pt-6 mb-8">
                ...
                <p className="text-[10px] text-gray-400 text-right mt-1 tracking-widest uppercase font-bold">
                  Total con impuestos incluidos
                </p>
              </div>

              {/* LÓGICA DE BOTONES COMBINADA */}
              {!mostrarPago ? (
                <>
                  <button 
                    ...
                    className={`w-full py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2
                      ${carritoTieneErrores 
                        ? 'bg-gray-300 text-gray-500 cursor-not-allowed' // Bloqueado por stock (Tu versión)
                ...
                <button 
                  ...
                  className="w-full bg-white border-2 border-gray-200 text-gray-600 py-4 rounded-xl font-bold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2"
                >
